...
import org.apache.oozie.command.jpa.WorkflowInfoWithActionsSubsetGetCommand;
...
public class JobXCommand extends WorkflowXCommand<WorkflowJobBean> {
    ...
    protected WorkflowJobBean execute() throws CommandException {
        try {
            ...
            if (jpaService != null) {
                this.workflow = jpaService.execute(new WorkflowInfoWithActionsSubsetGetCommand(this.id, this.start,
                        this.len));
            }
            else {
